...
const options = apiName => {
  return {
      ...
      headers: { "x-api-key": `${apiName}-${process.env.API_KEY}`, Authorization: "aws-self-bait"},
  }
}
...
const auth = axios.create( options("auth") )
auth.interceptors.request .use(bypass, rejector)
